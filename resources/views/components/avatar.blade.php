...
@if ($src)
    <img
        ...
        src="{{ $src }}"
        alt="{{ $name }}"
        {{ $attributes->class(['aspect-square object-cover border', $sizeClass, $rounded, $variantClass]) }}
    />
@else
    <div
        data-component-name="Avatar"
        {{ $attributes->class(['aspect-square flex items-center justify-center font-bold border', $sizeClass, $rounded, $variantClass]) }}
    >
        {{ $initial }}
    </div>
@endif
